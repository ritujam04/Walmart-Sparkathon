...
**API Usage**
**Auth**
POST /create_account — register champion (name, email, phone, address, pin_code, password)
POST /login — authenticate champion

**Availability**
POST /availability — champion posts selected availability date(s)
Request payload example:{ "champion_email": "xyz@example.com", "selected_date": "2025-12-15" }
Response example:{ "message": "Availability saved", "cutoff_at": "2025-12-10T18:00:00" }
...
**Orders & QR**
POST /place-order (User App) — creates order and returns order_id and qr_payload/qr_url to the customer
POST /orders/{order_id}/verify-qr — champion verifies scanned QR
Request example: { "champion_email": "xyz@example.com", "qr_payload": "<scanned_string>" }
